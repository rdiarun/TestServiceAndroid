...
namespace DetectorService
{
    ...
    public class login : Ilogin
    {
        public UserInfo ValidateTechnician(string emailId, string password)
        {
            ...
            {
                ds = Common.GetDataSet("p_GetUserByEmailPassword", param, paramValue);
                if (ds != null)
                {
                    ...
                    {                                                   //user name and password then send successful to the  
                        ...
                        lisTDetail.technicianName = ds.Tables[0].Rows[0].ItemArray[8].ToString();
                        lisTDetail.turnOffExpiryYear = Convert.ToBoolean(ds.Tables[0].Rows[0].ItemArray[9]);
                        lisTDetail.turnOffAlarmType = Convert.ToBoolean(ds.Tables[0].Rows[0].ItemArray[10]);
                        lisTDetail.turnOffNoOfAlarms = Convert.ToBoolean(ds.Tables[0].Rows[0].ItemArray[11]);
